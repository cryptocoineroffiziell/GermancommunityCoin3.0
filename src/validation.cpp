...
#if defined(NDEBUG)
# error "GermanCC Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	CAmount nSubsidy = consensusParams.nBlockRewardInit * COIN;
	int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
	    ...
	    if (halvings >= 33)
	    	...
	    	nSubsidy >>= halvings;
	    if(nHeight == 1 && consensusParams.nPremineCoins)
	    	nSubsidy = consensusParams.nPremineCoins * COIN;


    // Hard fork to reduce the block reward by 10 extra percent (allowing budget/superblocks)
    CAmount nSuperblockPart = (nHeight > consensusParams.nSuperblockStartBlock) ? nSubsidy * consensusParams.nBudgetPaymentsBlockSharePercent / 100 : 0;
...
{
	if(Params().GetConsensus().nMasternodeFixPaymentsPercent)
    	return Params().GetConsensus().nMasternodeFixPaymentsPercent * blockValue /100 ;
...
void ThreadScriptCheck() {
    RenameThread("germancc-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // GCC : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GCC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(phashdb->GetHash(block), GetTime()));
        return state.DoS(0, error("ConnectBlock(GCC): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // GCC : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(phashdb->GetHash(block), GetTime()));
                    return state.DoS(0, error("CheckBlock(GCC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(GCC): spork is off, skipping transaction locking checks\n");
    }
