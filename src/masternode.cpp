...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, int& nHeightRet)
{
    ...
    if(coin.out.nValue != consensusParams.nMasternodeCoins * COIN) { //germancctest
...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if(GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
        	...
        	if(out.nValue == consensusParams.nMasternodeCoins*COIN && out.scriptPubKey == payee) return true; //germancctest
    }
...
bool CMasternode::IsValidNetAddr(CService addrIn)
{
    ...
    return Params().NetworkIDString() == CBaseChainParams::REGTEST ||
    		...
    		((addrIn.IsIPv4() || addrIn.IsIPv6()) && IsReachable(addrIn) && addrIn.IsRoutable());// germancctest
}
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            //LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 GCC, masternode=%s\n", vin.prevout.ToStringShort());
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %u GCC, masternode=%s\n",consensusParams.nMasternodeCoins, vin.prevout.ToStringShort());//germacctest
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 GCC tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 GCC tx -> 1 confirmation
