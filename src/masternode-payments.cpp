...
*
*   Why is this needed?
*   - In GermanCC some blocks are superblocks, which output much higher amounts of coins
*   - Otherblocks are x% lower in outgoing value, so in total, no extra coins are created
...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string &strErrorRet)
{
    ...
    if(nBlockHeight < consensusParams.nSuperblockStartBlock) {
    	...
    	if(!isBlockRewardValueMet) {
    		LogPrint("gobject", "IsBlockValueValid -- no buget blocks before SuperblockStartBlock) \n");
    	                }
        return isBlockRewardValueMet; // no buget blocks before SuperblockStartBlock)
    }
...
{
    if(fLiteMode) return; // disable all GermanCC specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f GCC\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
